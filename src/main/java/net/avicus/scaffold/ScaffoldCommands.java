...
public class ScaffoldCommands {
    ...
    public ScaffoldCommands(Scaffold main) {
        this.instance = main;
    }
    ...
    @Command(aliases = "download", desc = "Download a world.", min = 0, max = 1, usage = "<world>")
    public static void download(CommandContext cmd, CommandSender sender) {
        ...
        String worldName = cmd.getString(0, current);
        ScaffoldWorld wrapper = ScaffoldWorld.ofSearch(worldName);
        File dir = new File(wrapper.getFolder().getAbsolutePath());

        String absolutePath = Bukkit.getServer().getWorldContainer().getAbsolutePath();
        File newDir = new File(absolutePath.substring(0, absolutePath.length() - 1) + "uploads/" + worldName);
        ...
        allowedFiles.add("region");
        allowedFiles.add("level.dat");
        allowedFiles.add("map.json"); // If JSON builder is added later on

        try {
            for (File file : newDir.listFiles()) {
                if (file.isDirectory() && !allowedFiles.contains(file.getName())) {
        ...
        Scaffold.instance().async(new Runnable() {
            ...
            public void run() {
                ...
                String randy = UUID.randomUUID().toString().substring(0, 6);
                File zip = new File(wrapper.getName() + "-" + randy + ".zip");

                try {
                    Zip.create(newDir, zip);
                } catch (Exception e) {
                    e.printStackTrace();
    ...
    @Command(aliases = "add", desc = "Add a world.", min = 2, max = 2, usage = "<.zip file link> <world name>")
